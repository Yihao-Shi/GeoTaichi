...
@ti.kernel
def resolve(total_contact: int):
    for n in range(total_contact):
        i = contact[n].end1
        j = contact[n].end2
...
cp_range = ti.field(dtype=ti.i32,shape=grid_n*grid_n)
contact = ti.Struct.field({"end1": int, "end2": int}, shape=100*n)
        ...
        for neigh_i in range(x_begin, x_end):
            for neigh_j in range(y_begin, y_end):
                ...
                for p_idx in range(grain_count[neigh_linear_idx]-list_cur[neigh_linear_idx],
                    ...
                    j = particle_id[p_idx]
                    if i < j:
                        ti.atomic_add(cp_range[neigh_linear_idx],1)
        ...
        for neigh_i in range(x_begin, x_end):
            for neigh_j in range(y_begin, y_end):
                ...
                for p_idx in range(grain_count[neigh_linear_idx]-list_cur[neigh_linear_idx],
                    ...
                    j = particle_id[p_idx]
                    if i < j:
                        offset=ti.atomic_sub(cp_range[neigh_linear_idx],1)
...
while step<5e4:
    for s in range(substeps):
        ...
        search()
        count_particle()
        cell_pse.run(cp_range)
        total_contact=cp_range[grid_n*grid_n-1]
        update_contact()
        resolve(total_contact)
    step+=1
    pos = gf.p.to_numpy()
