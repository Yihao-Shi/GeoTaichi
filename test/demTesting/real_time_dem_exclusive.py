...
def contact(gf: ti.template()):
    ...
    for i in range(n):
        ...
        verlet_table_in_target_cell(i, grid_idx[0], grid_idx[1])
        verlet_table(i, grid_idx[0]-1, grid_idx[1])
        verlet_table(i, grid_idx[0]-1, grid_idx[1]-1)
        verlet_table(i, grid_idx[0], grid_idx[1]-1)
        verlet_table(i, grid_idx[0]+1, grid_idx[1]-1)

@ti.func
def verlet_table_in_target_cell(i, neigh_i, neigh_j):
    neigh_linear_idx = neigh_i * grid_n + neigh_j
    if 0 <= neigh_linear_idx < grid_n * grid_n:
...
@ti.func
def verlet_table(i, neigh_i, neigh_j):
    neigh_linear_idx = neigh_i * grid_n + neigh_j
    if 0 <= neigh_linear_idx < grid_n * grid_n:
        for p_idx in range(list_head[neigh_linear_idx],  list_tail[neigh_linear_idx]):
            j = particle_id[p_idx]
