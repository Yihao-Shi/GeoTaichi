...
facet[1].add_wall_geometry(wallID=0,vertice1=ti.Vector([0., 6., 1.1]),
...
def area_fraction(facet: ti.template(), position: ti.types.vector(3, float), radius: float):
    area1 = facet[0].processCircleShape(position, radius)
    area2 = facet[1].processCircleShape(position, radius)
