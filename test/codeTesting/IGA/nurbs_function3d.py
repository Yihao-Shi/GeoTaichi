...
for c in range(xi.shape[0]*eta.shape[0]):
    i = c % xi.shape[0]
    j = c // xi.shape[0]
    result1 = nurbs_interpolation2d.nurbs_interpolation2d(xi[i], eta[j], p, q, knot_u, knot_v, points[:,0], weights)
    result2 = nurbs_interpolation2d.nurbs_interpolation2d(xi[i], eta[j], p, q, knot_u, knot_v, points[:,1], weights)
    result3 = nurbs_interpolation2d.nurbs_interpolation2d(xi[i], eta[j], p, q, knot_u, knot_v, points[:,2], weights)
    interpolatedPoints[c, :] = [result1[0], result2[0], result3[0]]

fig = plt.figure(figsize=(6,6))
ax3 = plt.axes(projection='3d')
plt.rcParams['axes.unicode_minus']=False
ax3.plot_surface(np.reshape(interpolatedPoints[:,0], (eta.shape[0], xi.shape[0])),np.reshape(interpolatedPoints[:,1], (eta.shape[0], xi.shape[0])),np.reshape(interpolatedPoints[:,2], (eta.shape[0], xi.shape[0])),cmap=plt.cm.cool) 

ax3.scatter(points[:,0], points[:,1], points[:,2], marker="*", color="red")
plt.show()
plt.close()
