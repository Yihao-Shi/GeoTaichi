...
def assign_displacement_constraints():
    for i in g:
        ...
        if idx <= space:
            disp_constraint[3 * i].value = 0.
            disp_constraint[3 * i].dof = 1
            disp_constraint[3 * i + 1].value = 0.
            disp_constraint[3 * i + 1].dof = 1
            disp_constraint[3 * i + 2].value = 0.
            disp_constraint[3 * i + 2].dof = 1
...
def compute_mass_list():
    ...
    for grid_id in g:
        if g[grid_id].m > val_lim:
            a = MAXVAL * disp_constraint[3 * grid_id].dof + 1
            b = MAXVAL * disp_constraint[3 * grid_id + 1].dof + 1
            c = MAXVAL * disp_constraint[3 * grid_id + 2].dof + 1  
            gm[g[grid_id].dof[0]] = a * (g[grid_id].m / dt / dt / beta)
            gm[g[grid_id].dof[1]] = b * (g[grid_id].m / dt / dt / beta)
            gm[g[grid_id].dof[2]] = c * (g[grid_id].m / dt / dt / beta)
...
def assemble_residual_force():
    for grid_id in g:
        if g[grid_id].m > val_lim:
            ...
            if disp_constraint[3 * grid_id].dof == 1:
                b[g[grid_id].dof[0]] = MAXVAL * disp_constraint[3 * grid_id].value * M[g[grid_id].dof[0]]
            if disp_constraint[3 * grid_id + 1].dof == 1:
                b[g[grid_id].dof[1]] = MAXVAL * disp_constraint[3 * grid_id + 1].value * M[g[grid_id].dof[1]]
            if disp_constraint[3 * grid_id + 2].dof == 1:
                b[g[grid_id].dof[2]] = MAXVAL * disp_constraint[3 * grid_id + 2].value * M[g[grid_id].dof[2]]
        ...
        for j in range(offset[i]):
            ...
            for k in range(offset[i]):
                ...
                if LnID[i, j] == LnID[i, k]:
                    a = MAXVAL * disp_constraint[3 * LnID[i, k]].dof + 1
                    b = MAXVAL * disp_constraint[3 * LnID[i, k] + 1].dof + 1
                    c = MAXVAL * disp_constraint[3 * LnID[i, k] + 2].dof + 1
                local_stiffness[base_offset * ifnode * ifnode + 3 * j * ifnode + 3 * k] += a * (ijdshape[0] * ikdshape[0] * a1 + (ijdshape[1] * ikdshape[1] + ijdshape[2] * ikdshape[2]) * shear) * pvol
                local_stiffness[base_offset * ifnode * ifnode + 3 * j * ifnode + 3 * k + 1] += (ijdshape[0] * ikdshape[1] * a2 + ijdshape[1] * ikdshape[0] * shear) * pvol
                local_stiffness[base_offset * ifnode * ifnode + 3 * j * ifnode + 3 * k + 2] += (ijdshape[0] * ikdshape[2] * a2 + ijdshape[2] * ikdshape[0] * shear) * pvol
                local_stiffness[base_offset * ifnode * ifnode + (3 * j + 1) * ifnode + 3 * k] += (ijdshape[1] * ikdshape[0] * a2 + ijdshape[0] * ikdshape[1] * shear) * pvol
                local_stiffness[base_offset * ifnode * ifnode + (3 * j + 1) * ifnode + 3 * k + 1] += b * (ijdshape[1] * ikdshape[1] * a1 + (ijdshape[0] * ikdshape[0] + ijdshape[2] * ikdshape[2]) * shear) * pvol
                local_stiffness[base_offset * ifnode * ifnode + (3 * j + 1) * ifnode + 3 * k + 2] += (ijdshape[1] * ikdshape[2] * a2 + ijdshape[2] * ikdshape[1] * shear) * pvol
                local_stiffness[base_offset * ifnode * ifnode + (3 * j + 2) * ifnode + 3 * k] += (ijdshape[2] * ikdshape[0] * a2 + ijdshape[0] * ikdshape[2] * shear) * pvol
                local_stiffness[base_offset * ifnode * ifnode + (3 * j + 2) * ifnode + 3 * k + 1] += (ijdshape[2] * ikdshape[1] * a2 + ijdshape[1] * ikdshape[2] * shear) * pvol
                local_stiffness[base_offset * ifnode * ifnode + (3 * j + 2) * ifnode + 3 * k + 2] += c * (ijdshape[2] * ikdshape[2] * a1 + (ijdshape[0] * ikdshape[0] + ijdshape[1] * ikdshape[1]) * shear) * pvol
...
def cg(v: ti.template(), mv: ti.template()):
    ...
    num_block = (ifnode * active_id[None]) // BLOCK_SZ
    for i in range(ifnode * active_id[None]):
        thread_id = i % BLOCK_SZ
        block_id = int(i // BLOCK_SZ)
        pprod = ti.simt.block.SharedArray((BLOCK_SZ * ifnode, ), ti.f64)
        ...
        if block_id == num_block:
            temp = (ifnode * active_id[None])%BLOCK_SZ
        
        lane = base_offset * ifnode * ifnode + dofID 
        vector_val = v[g[nodeID1].dof[local_dof1]]
        for j in range(ifnode):
            pprod[thread_id + j * temp] += local_stiffness[lane] * vector_val
            lane += ifnode
        ti.simt.block.sync()
        ...
        for j in range(ifnode):
            ...
            mv[g[nodeID2].dof[local_dof2]] += pprod[thread_id + j * ifnode]
...
def solve(A, matrix_free):
    ...
    #  packing1 = ifnode, ifnode, ****
    #  packing2 = BLOCK_SIZE, ifnode, ****
    ...
    while not convergence and iter_num < iter_max:
        ...
        inv_diag_A()
        matrix_free.solve(A, b, x, M, total_nodes, maxiter=10 * total_nodes, tol=1e-15)
