...
def compute_stress_strain():
    '''start = 3 * flag[flag.shape[0] - 1]
    for i in g:
        if g[i].dof[2] >= 0:
            print(i, 2, x[g[i].dof[2]], x[start + g[i].dof[2]])
    print('\n')'''
    for i in range(n_particles):
...
def assemble_residual_force():
    ...
    for i in range(disp_list[None]):
        ...
        if g[nodeID].dof[dofID] >= 0:
            b[start + g[nodeID].dof[dofID]] = disp_constraint[i].value - g[nodeID].disp[dofID]
...
@ti.kernel
def assemble_displacement_force():
    start = 3 * flag[flag.shape[0] - 1]
    ...
    for i in range(n_particles):
        ...
        for j in range(offset[i]):
            ...
            for k in range(offset[i]):
                ikdshape = dshape[i, k]
                local_stiffness[base_offset * ifnode * ifnode + 3 * j + (3 * k) * ifnode] += (ijdshape[0] * ikdshape[0] * a1 + (ijdshape[1] * ikdshape[1] + ijdshape[2] * ikdshape[2]) * shear) * pvol
                ...
                local_stiffness[base_offset * ifnode * ifnode + (3 * j + 1) + (3 * k + 1) * ifnode] += (ijdshape[1] * ikdshape[1] * a1 + (ijdshape[0] * ikdshape[0] + ijdshape[2] * ikdshape[2]) * shear) * pvol
                ...
                local_stiffness[base_offset * ifnode * ifnode + (3 * j + 2) + (3 * k + 2) * ifnode] += (ijdshape[2] * ikdshape[2] * a1 + (ijdshape[0] * ikdshape[0] + ijdshape[1] * ikdshape[1]) * shear) * pvol
...
def cg(v: ti.template(), mv: ti.template()):
    ...
    for i in range(disp_list[None]):
        ...
        dofID = disp_constraint[i].dof % 3
        if g[nodeID].dof[dofID] >= 0:
            mv[g[nodeID].dof[dofID]] += v[start + g[nodeID].dof[dofID]]
            mv[start + g[nodeID].dof[dofID]] += v[g[nodeID].dof[dofID]]
...
def solve(A, matrix_free):
    ...
    while not convergence and iter_num < iter_max:
        ...
        inv_diag_A()
        matrix_free.solve(A, b, x, M, 2*total_nodes, maxiter=20 * total_nodes, tol=1e-5)
