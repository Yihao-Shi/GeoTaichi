...
from geotaichi.utils.MaterialKernel import MeanStress, calculate_strain_increment
from geotaichi.utils.constants import Threshold, ZEROMAT4x4, ZEROMAT6x3, ZEROVEC3f, ZEROMAT3x3, DELTA, EYE
from geotaichi.utils.MatrixFunction import truncation
from geotaichi.utils.ShapeFunctions import ShapeLinear, GShapeLinear
from geotaichi.utils.TypeDefination import vec3f, vec4f, vec6f, mat4x4, vec3i
from geotaichi.utils.VectorFunction import Normalize, outer_product, MeanValue, voigt_form, packingIndex
...
@ti.kernel
def kernel_reference_internal_force_p2g(total_nodes: int, particleNum: int, node: ti.template(), particle: ti.template(), stateVars: ti.template(), LnID: ti.template(), dshapefn: ti.template(), node_size: ti.template()):
    for np in range(particleNum):
        if int(particle[np].materialID) > 0 and int(particle[np].active) == 1:
            bodyID = int(particle[np].bodyID)
            offset = np * total_nodes
            ...
            for ln in range(offset, offset + int(node_size[np])):
                nodeID = LnID[ln]
                internal_force = dshapefn[ln] @ fInt
                node[nodeID, bodyID]._update_internal_force(internal_force)
...
def kernel_compute_reference_stress_strain(total_nodes: int, dt: ti.template(), particleNum: int, node: ti.template(), particle: ti.template(), 
    ...
    for np in range(particleNum):
        ...
        if materialID > 0 and int(particle[np].active) == 1:
            ...
            stress = matPorps[materialID].ComputePKStress(np, previous_stress, velocity_gradient, stateVars, dt)
...
def kernel_compute_stress_strain_newmark(total_nodes: int, dt: ti.template(), particleNum: int, node: ti.template(), particle: ti.template(), 
    ...
    for np in range(particleNum):
        ...
        if materialID > 0 and int(particle[np].active) == 1:
            ...
            particle[np].vol = particle[np].vol0 * matPorps[materialID].update_particle_volume(np, velocity_gradient, stateVars, dt)
...
def kernel_update_stress_strain_newmark(particleNum: int, particle: ti.template(), dt: ti.template()):
    ...
    for np in range(particleNum):
        materialID = int(particle[np].materialID)
        if materialID > 0 and int(particle[np].active) == 1:
            particle[np].strain += calculate_strain_increment(particle[np].velocity_gradient, dt)
