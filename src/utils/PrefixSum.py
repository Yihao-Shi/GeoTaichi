...
class PrefixSumExecutor:
    ...
    """
    def __init__(self, length, level=1):
        self.array_length = []
        self.level = level
        self.large_arr = None
        self.npad = []
    ...
    def initialize(self, length):
        ...
        if current_cfg().arch == ti.cuda:
            ...
            for l in range(self.level):
                if length[l] > 2:
                    self.large_arr = ti.field(int, shape=next_pow2(length[l] + 1))
    ...
    def runGPU(self, input_arr, nlevel=0):
        ...
        for i in range(len(ele_nums) - 1):
            if i == len(ele_nums) - 2:
                scan_add_inclusive(input_arr, ele_nums_pos[i], ele_nums_pos[i + 1], 1)
            else:
                scan_add_inclusive(input_arr, ele_nums_pos[i], ele_nums_pos[i + 1], 0)
        
        for i in range(len(ele_nums) - 3, -1, -1):
            uniform_add(input_arr, ele_nums_pos[i], ele_nums_pos[i + 1])
