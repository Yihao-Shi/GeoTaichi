...
class ParticleCreator(object):
    ...
    def create(self, sims, scene, body_dict):
        ...
        if btype == "Sphere":
            ...
            self.create_rigid_sdf(sims, scene, template)
        else:
    ...
    def create_template_rigid_body(self, sims, scene: myScene, template):
        ...
        grid = scene.get_grid_ptr()
        surface_node = scene.get_surface_node()
        rigid_body = scene.get_rigid_ptr()
        material = scene.get_material_ptr()
        gridNum = int(scene.gridNum[0])
        rigidNum = int(scene.rigidNum[0])
        ...
        scene.check_rigid_number(sims, body_number=1)
        scene.check_grid_number(sims, grid_number=template_ptr.grid.gridSum)
        scene.check_surface_node_number(sims, surface_node_number=template_ptr.surface_node.shape[0])

        kernel_create_level_set_body_(rigid_body, surface_node, grid, bounding_box, bounding_sphere, material, rigidNum, gridNum, surfaceNum, 
                                      template_ptr.boundingSphere.r_bound, vec3f(template_ptr.boundingSphere.x_bound), template_ptr.surface_node, template_ptr.grid.node_coord, template_ptr.grid.distance_field,
                                      com_pos, equiv_rad, set_orientations.get_orientation, groupID, matID, init_v, init_w)
        print(" Level set body Information ".center(71, '-'))
        self.print_particle_info(groupID, matID, com_pos, init_v, init_w, name=name)

        scene.rigidNum[0] += 1
        scene.gridNum[0] += template_ptr.grid.gridSum
        scene.surfaceNum[0] += template_ptr.surface_node.shape[0]
        
    def create_rigid_sdf(self, sims, scene: myScene, template):
        if type(template) is dict:
            self.create_template_soft_sdf(sims, scene, template)
        elif type(template) is list:
            for temp in template:
                self.create_template_soft_sdf(sims, scene, temp)

    def create_template_soft_sdf(self, sims, scene: myScene, template):
        pass
