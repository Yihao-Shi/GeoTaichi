...
from src.dem.BaseStruct import VerletContactTable
...
class NeighborBase(object):
    ...
    def manage_function_base(self, scene: myScene):
        ...
        if not self.sims.wall_type is None and self.sims.wall_type != 3 and self.sims.max_particle_num > 0:
            if self.sims.scheme == "DEM":
        ...
        if self.sims.scheme == "LSDEM":
            self.update_verlet_tables_point_particle = self.no_operation
            self.update_verlet_tables_point_wall = self.no_operation
            if self.sims.max_particle_num > 1:
                self.update_verlet_tables_point_particle = self.update_verlet_table_lsparticle_lsparticle_scenod_layer
            if self.sims.max_wall_num > 0 and self.sims.wall_type != 3:
                self.update_verlet_tables_point_wall = self.update_verlet_table_lsparticle_wall_scenod_layer

            self.reset_relative_displacements = self.no_operation
            if self.sims.max_particle_num > 1:
    ...
    def set_potential_contact_list(self, scene):
        if self.sims.max_particle_num > 1:
            if self.sims.scheme == "DEM":
                ...
                self.potential_list_point_particle = ti.field(int, shape=self.sims.potential_contact_points_particle * self.sims.max_rigid_body_num)
                self.particle_particle = ti.field(int, shape=self.particle_pse.get_length())
        ...
        if not self.sims.wall_type is None:
            if self.sims.wall_type != 3 and self.sims.max_particle_num > 0:
                if self.sims.scheme == "DEM":
                ...
                if self.sims.scheme == "LSDEM":
                    self.potential_list_point_wall = ti.field(int, shape=self.sims.potential_contact_points_wall * self.sims.max_rigid_body_num)
                    self.particle_wall = ti.field(int, shape=self.particle_pse.get_length())
