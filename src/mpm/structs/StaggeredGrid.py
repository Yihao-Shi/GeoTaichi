...
from src.utils.constants import DBL_EPSILON, ZEROVEC3f
from src.utils.ScalarFunction import vectorize_id, linearize3D
from src.utils.TypeDefination import vec3i, vec3f
from src.utils.Quaternion import SetToRotate
from src.utils.VectorFunction import Squared, SquaredLength, inner_multiply


@ti.data_oriented
class StaggeredGrid:
    def __init__(self, sims: Simulation, cnum, gridSum) -> None:
        self.dimension = sims.dimension

        indice = ti.ij if sims.dimension == 2 else ti.ijk
        self.cell_type = ti.field(dtype=ti.u8)
        self.pressure = ti.field(dtype=float)
        self.tension = ti.Vector.field(sims.dimension, dtype=float)
        if sims.coupling:
            self.fraction = ti.field(dtype=float)
            ti.root.dense(indice, cnum).place(self.cell_type, self.pressure, self.tension, self.fraction)
        else:
            ti.root.dense(indice, cnum).place(self.cell_type, self.pressure, self.tension)
        self.velocity = [ti.field(dtype=float, shape=([cnum[i] + (d == i) for i in range(sims.dimension)])) for d in range(sims.dimension)]
        self.grid_m = [ti.field(dtype=float, shape=([cnum[i] + (d == i) for i in range(sims.dimension)])) for d in range(sims.dimension)]
        self.valid = ti.field(dtype=int, shape = ([cnum[i] + 1 for i in range(sims.dimension)])) 
        self.shape = np.array([gridSum, 1])
        if sims.coupling:
