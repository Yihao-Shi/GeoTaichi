...
from src.utils.constants import DELTA, PI, FTOL, LTOL, MAXITS, NSUB, EPS, STOL, dTmin, DELTA2D
...
class ULStateVariable:
    ...
    def _initialize_vars(self, np, particle, matProps):
        ...
        self.void_ratio = matProps[materialID].e0

    @ti.func
    def _update_vars(self, stress, epstrain, void_ratio):
        self.estress = VonMisesStress(stress)
...
@ti.dataclass
class StateDependentMohrCoulombModel:
    density: float
    young: float
    possion: float
    e0: float
    ...
    def add_material(self, density, young, possion, e0, e_Tao, lambda_c, ksi, nd, nf, fai_c, c, tensile):
    ...
    def ComputeStress(self, np, previous_stress, velocity_gradient, stateVars, dt):  
        ...
        previous_stress = self.ImplicitIntegration1(np, previous_stress, de, dw, stateVars)
        return previous_stress
    ...
    @ti.func
    def StateDependent(self, np, p, dv, stateVars):
        e_Tao = self.e_Tao
        ...
        # stateVars[np].void_ratio = (1.0 + e)* (1.0 + dv) - 1.0
        stateVars[np].void_ratio = e + (1.0 + e) * dv
        # stateVars[np].void_ratio = e0 - (1 + e0) * (1.0 - dv)
        ...
        fai = ti.atan2(fai, 1.)
        # if fai <=0:
        #     print(SP, stateVars[np].void_ratio, dv, p)
        if psi < 0.:
            psi = 0.
        return fai, psi
    ...
    def ImplicitIntegration(self, np, previous_stress, de, dw, stateVars):
        ...
        if p < 1000.:
            p = 1000.
        dv = de[0] + de[1] + de[2]
        self.fai, self.psi = self.StateDependent(np, p, dv, stateVars)

        if self.fai == 0:
            self.tensile = 0.
        else:
            self.tensile = ti.min(self.tensile, self.c / ti.tan(self.fai))
    ...
    @ti.func
    def ImplicitIntegration1(self, np, previous_stress, de, dw, stateVars):
        e_Tao = self.e_Tao
        lambda_c = self.lambda_c
        ksi = self.ksi
        fai_c = self.fai_c
